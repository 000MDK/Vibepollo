...
namespace platf::audio {
  ...
  int mic_write_wasapi_t::init() {
    if (audio_client || opus_decoder != nullptr) {
      // init() must be idempotent: if it's called again on an already-initialized
      // backend (e.g. a stream restarting without an intervening cleanup()), tear
      // down the previous device/decoder/thread first. Otherwise opus_decoder and
      // the COM device objects leak, and assigning a new std::thread over the
      // still-joinable render_thread below would call std::terminate().
      cleanup();
    }
    ...
    while (!stop_render_thread) {
      ...
      if (FAILED(status)) {
        BOOST_LOG(debug) << "Couldn't query microphone playback padding for [" << target_device_name << "]: 0x"
                         << util::hex(status).to_string_view();
        if (is_recoverable_device_error(status)) {
          if (!recover_device(*this, status, "querying playback padding")) {
            ::audio::mic_debug_on_backend_error("Steam microphone playback device was invalidated during rendering and could not be recovered. Restart the stream.");
          }
          // Either recover_device() spun up a fresh render thread that now owns
          // this object, or it failed and cleanup() already tore everything down.
          // Either way this thread is done; break out to CoUninitialize() and exit.
          break;
        }
        continue;
      }
      ...
      if (FAILED(status) || buffer == nullptr) {
        ...
        if (FAILED(status) && is_recoverable_device_error(status)) {
          if (!recover_device(*this, status, "acquiring a render buffer")) {
            ::audio::mic_debug_on_backend_error("Steam microphone playback device was invalidated while acquiring a render buffer and could not be recovered. Restart the stream.");
          }
          break;
        }
        continue;
      ...
      if (FAILED(status)) {
        BOOST_LOG(debug) << "Couldn't release microphone playback buffer for [" << target_device_name << "]: 0x"
                         << util::hex(status).to_string_view();
        if (is_recoverable_device_error(status)) {
          if (!recover_device(*this, status, "releasing a render buffer")) {
            ::audio::mic_debug_on_backend_error("Steam microphone playback device was invalidated while releasing a render buffer and could not be recovered. Restart the stream.");
          }
          break;
        }
      }
  ...
  void mic_write_wasapi_t::cleanup() {
    ...
    if (render_thread.joinable()) {
      if (render_thread.get_id() == std::this_thread::get_id()) {
        // cleanup() can be invoked from inside the render thread itself while
        // recovering from a device error (see recover_device()/render_loop()).
        // Joining our own thread would deadlock, so detach instead; render_loop()
        // is about to return on its own, and detach() makes render_thread
        // non-joinable so a later reinit can safely assign a new thread to it.
