...
namespace platf::audio {
  ...
  private:
    ...
    bool install_driver_from_local_steam_inf(const wchar_t *driver_path_template, std::wstring_view driver_name, bool restore_default_roles) {
#ifdef STEAM_DRIVER_SUBDIR
      ...
      // only some of the current policy endpoints. Only needed when we intend
      // to restore roles afterward (e.g. not for the microphone driver, which
      // doesn't affect the default output device).
      role_device_ids_t old_default_ids;
      if (restore_default_roles) {
